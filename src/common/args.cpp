...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bitpty";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bitpty";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bitpty";
#else
    // Unix-like
    return pathRet / ".bitpty";
#endif
#endif
}
